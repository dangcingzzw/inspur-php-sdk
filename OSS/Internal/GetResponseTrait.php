...
use OSS\Internal\Common\CheckoutStream;
...
trait GetResponseTrait
{
						...
						if(isset($value['type']) && $value['type'] === 'stream'){
                            ...
                            if ($jsonBody && ($data = json_decode($jsonBody, true))) {
                                if (is_array($data)) {
                                    ...
                                    OSSLog::commonLog(ERROR, "response body %s, and jsonToArray data is %s",$body, $data);
                                }
                            }
                            $closeBody = true;
                        } else {
		...
		if(is_string($filePath) && $filePath !== '')
		{
			...
			try{
				...
				if(($fp = fopen($filePath, 'w')))
				{
					...
					fflush($fp);
					OSSLog::commonLog(DEBUG, "write file %s ok",$filePath);
				}
				else{
					OSSLog::commonLog(ERROR, "open file error,file path:%s",$filePath);
				}
			}finally{
				if($fp){
					fclose($fp);
	...
	private function parseXmlToException($body, $OSSException){
		try{
			$xmlErrorBody = trim($body -> getContents());
			if($xmlErrorBody && ($xml = simplexml_load_string($xmlErrorBody))){
				$prefix = $this->getXpathPrefix($xml);
				if ($tempXml = $xml->xpath('//'.$prefix . 'Code')) {
					$OSSException-> setExceptionCode(strval($tempXml[0]));
				}
				if ($tempXml = $xml->xpath('//'.$prefix . 'RequestId')) {
					$OSSException-> setRequestId(strval($tempXml[0]));
				}
				if ($tempXml = $xml->xpath('//'.$prefix . 'Message')) {
					$OSSException-> setExceptionMessage(strval($tempXml[0]));
				}
				if ($tempXml = $xml->xpath('//'.$prefix . 'HostId')) {
					$OSSException -> setHostId(strval($tempXml[0]));
				}
			}
		}finally{
			$body -> close();
	...
	private function parseJsonToException($body, $OSSException) {
        try {
            $jsonErrorBody = trim($body -> getContents());
            if ($jsonErrorBody && ($data = json_decode($jsonErrorBody, true))) {
                if (is_array($data)) {
                    if ($data['request_id']) {
                        $OSSException -> setRequestId(strval($data['request_id']));
                    }
                    if ($data['code']) {
                        $OSSException -> setExceptionCode(strval($data['code']));
                    }
                    if ($data['message']) {
                        $OSSException -> setExceptionMessage(strval($data['message']));
                    }
                } elseif (strlen($data)) {
                    OSSLog::commonLog(ERROR, "response body %s, and jsonToArray data is %s",$body, $data);
                    $OSSException-> setExceptionMessage("Invalid response data，since it is not json data");
                }
            }
        } finally {
            $body -> close();
	    ...
	    try{
	        ...
	        if ($jsonErrorBody && ($jsonArray = json_decode($jsonErrorBody, true))) {
	            if (is_array($jsonArray)) {
                    ...
                    OSSLog::commonLog(ERROR, "response body %s, and jsonToArray data is %s",$body, $jsonArray);
	...
	protected function parseResponse(Model $model, Request $request, Response $response, array $requestConfig)
	{
		...
		if($statusCode >= 300){
			if($this-> exceptionResponseMode){
				...
				$OSSException-> setRequest($request);
				$OSSException-> setResponse($response);
				$OSSException-> setExceptionType($this->isClientError($response) ? 'client' : 'server');
				if ($responseContentType === 'application/json') {
				    $this->parseJsonToException($body, $OSSException);
                } else {
                    $this->parseXmlToException($body, $OSSException);
                }
				throw $OSSException;
			}else{
				$this->parseCommonHeaders($model, $response);
	...
	protected function buildException(Request $request, RequestException $exception, $message)
	{
		...
		$OSSException= new OSSException($message ? $message : $exception-> getMessage());
		...
		if($response){
			...
			if ($this->isJsonResponse($response)) {
                $this->parseJsonToException($response -> getBody(), $OSSException);
            } else {
                $this->parseXmlToException($response -> getBody(), $OSSException);
            }
    		if ($OSSException->getRequestId() === null) {
    		    $prefix = strcasecmp($this->signature, 'OSS' ) === 0 ? 'x-OSS-' : 'x-amz-';
    		    $requestId = $response->getHeaderLine($prefix . 'request-id');
    		    $OSSException->setRequestId($requestId);
    		}
		}
		return $OSSException;
	}
