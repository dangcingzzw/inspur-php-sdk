...
namespace OSS\Internal\Signature;

use OSS\Log\OSSLog;
use OSS\Internal\Resource\Constants;
use OSS\OSSException;
use OSS\Internal\Common\SchemaFormatter;
use GuzzleHttp\Psr7\Stream;
use OSS\Internal\Common\Model;
use Psr\Http\Message\StreamInterface;
use OSS\Internal\Common\OSSTransform;
use OSS\Internal\Common\V2Transform;
...
abstract class AbstractSignature implements SignatureInterface
{
			...
			if($type === 'array'){
				...
				foreach ($properties as $pkey => $pvalue){
					if(isset($pvalue['required']) && $pvalue['required'] && !isset($subParams[$pkey])){
						$OSSException= new OSSException('param:' .$pkey. ' is required');
		...
		if($treatAsString){
			if($type === 'boolean'){
				if(!is_bool($subParams) && strval($subParams) !== 'false' && strval($subParams) !== 'true'){
					$OSSException= new OSSException('param:' .$key. ' is not a boolean value');
					$OSSException-> setExceptionType('client');
					throw $OSSException;
				}
			}else if($type === 'numeric'){
				if(!is_numeric($subParams)){
					$OSSException= new OSSException('param:' .$key. ' is not a numeric value');
					$OSSException-> setExceptionType('client');
					throw $OSSException;
				}
			}else if($type === 'float'){
				if(!is_float($subParams)){
					$OSSException= new OSSException('param:' .$key. ' is not a float value');
					$OSSException-> setExceptionType('client');
					throw $OSSException;
				}
			}else if($type === 'int' || $type === 'integer'){
				if(!is_int($subParams)){
					$OSSException= new OSSException('param:' .$key. ' is not a int value');
					$OSSException-> setExceptionType('client');
					throw $OSSException;
				}
			}
	...
	{
	    $transHolder = strcasecmp($this-> signature, 'OSS') === 0 ? OSSTransform::getInstance() : V2Transform::getInstance();
		...
		if(isset($requestConfig['requestParameters'])){
			...
			foreach ($paramsMetadata as $key => $value){
				if(isset($value['required']) && $value['required'] && !isset($params[$key])){
					$OSSException= new OSSException('param:' .$key. ' is required');
					$OSSException-> setExceptionType('client');
					throw $OSSException;
				}
				if(isset($params[$key]) && isset($value['location'])){
					...
					if($location === 'header'){
						...
						if(isset($result['body'])){
							$OSSException= new OSSException('duplicated body provided');
							$OSSException-> setExceptionType('client');
							throw $OSSException;
						}
						...
						if($type === 'file'){
							if(!file_exists($val)){
								$OSSException= new OSSException('file[' .$val. '] does not exist');
								$OSSException-> setExceptionType('client');
								throw $OSSException;
							}
                            ...
                            if (!$jsonData) {
                                $OSSException= new OSSException('input is invalid, since it is not json data');
                                $OSSException-> setExceptionType('client');
                                throw $OSSException;
                            }
		...
		if($xml || (isset($requestConfig['data']['xmlAllowEmpty']) && $requestConfig['data']['xmlAllowEmpty'])){
			...
			OSSLog::commonLog(DEBUG, 'request content ' . $result['body']);
